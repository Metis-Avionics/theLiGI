...
`@specs/SPEC.toml` enforce these rules. A safety-critical change that
...
- **Recursion** — forbidden in safety-critical crates (`theligi-inference`,
  `theligi-knowledge`, `theligi-content`, `theligi-validation`,
  `theligi-evidence`). All functions must be statically non-recursive. The
  acyclic call graph this guarantees enables bounded-execution proofs.
...
- Spec: `@specs/SUBSYSTEM.toml [invariants] no_cross_runtime = true`.
...
- `loop { ... }` is forbidden in safety-critical code unless it is a
  non-terminating scheduler and explicitly marked as such with a spec
  reference.
...
- Spec: `@specs/SUBSYSTEM.toml [scheduler] requires = [retry, dead_letter]`.
...
- **Server/desktop crates** (`theligi-server`, `theligi-cli`,
  `theligi-desktop`) — heap allocation is permitted; these are not
  safety-critical.

- **Intelligence crates** (`theligi-inference`, `theligi-knowledge`,
  `theligi-content`, `theligi-validation`, `theligi-evidence`) — no
...
- **`no_std`** — not currently required. The single-runtime constraint
  (`SPEC.toml [runtime] runtime = "single_process_rust"`) is the
  current boundary.
...
- Spec: `@specs/SPEC.tomL [architecture] runtime = "single_process_rust"`.
...
- Spec: `@specs/SPEC.toml [development] public_api_documentation = true`
  (forces function-level doc comments which count toward the
...
- Spec: `@specs/SPEC.toml [safety] fail_closed = true` +
  `@specs/SUBSYSTEM.toml [content] invariants`.
...
- **Parameter validation** — every public function in a safety-critical
  ...
  - Content artifact sequence bounds (for series generation).
  - Evidence hierarchy thresholds (for sufficiency checks).
...
- **Procedural macros** — allowed only in `theligi-artifact` (if needed
  for derive macros) and build scripts. Not in the inference/content
  control loop.
...
- Spec: `@specs/SPEC.toml [security] fail_closed = true`.
...
- **Rustc warnings** — `#![deny(warnings)]` in safety-critical crates.
- **Clippy** — `cargo clippy --workspace --all-targets -- -D warnings`
  with `clippy::pedantic` enabled at the crate level for safety-critical
  crates.
...
The first four are required by `@specs/SPEC.toml [development]` and
`AGENTS.md`. The last four are required by TETANUS.md for
safety-critical crates.

### Workspace lint configuration

Each safety-critical crate (`theligi-inference`, `theligi-knowledge`,
`theligi-content`, `theligi-validation`, `theligi-evidence`,
`theligi-data_access`, `theligi-repository`) must have at the top of
`lib.rs`:
...
- `theligi-inference` — GNN inference engine
- `theligi-knowledge` — Oxigraph + HelixDB graph operations
- `theligi-content` — content series FSM + artifact generation
- `theligi-validation` — series invariant enforcement
- `theligi-evidence` — evidence sufficiency + lineage hash chain
- `theligi-data_access` — data pipeline orchestration
- `theligi-repository` — CAS + generation mutation

The following crates are not safety-critical but should follow rules 4,
6, 7, 10 as general quality practice:

- `theligi-core`, `theligi-schema`, `theligi-context`
- `theligi-message`, `theligi-query`, `theligi-runtime`
- `theligi-cache`, `theligi-algorithm`, `theligi-factory`
- `theligi-topic`, `theligi-graph`, `theligi-sparql`
...
- `theligi-scheduler`, `theligi-telemetry`, `theligi-feedback`
- `theligi-artifact`, `theligi-security`, `theligi-observability`
- `theligi-server` (binary), `theligi-cli` (binary), `theligi-desktop` (binary)
